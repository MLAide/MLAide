...
import { ArtifactsApiService, ExperimentsApiService, GraphEdge, GraphNode, LineageGraphUiService, RunsApiService } from 'src/app/core/services';
...
import { RunsListTableComponent } from '../../shared/runs-list-table/runs-list-table.component';
...
describe('ExperimentDetailsComponent', () => {
  ...
  describe('ngOnInit', () => {
    ...
    it('should load all artifacts with the run keys of the current experiment', async () => {
      ...
      expect(artifactsApiServiceStub.getArtifactsByRunKeys).toHaveBeenCalledWith(fakeProject.key, fakeRuns.map(r => r.key));
    });
    ...
    it('should map all runs to nodes and edges and call the lineage graph service', async () => {
      ...
      expect(lineageGraphServiceStub.renderLineage).toHaveBeenCalledWith(
        expectedNodes, expectedEdges, component.experimentGraphSvg);
    });
  });
  ...
  function setupRunsStub(runs: Run[]) {
    // setup runs api
    runsApiServiceStub.getRunsByExperimentKey.withArgs(fakeProject.key, fakeExperiment.key).and.returnValue(runListDataSourceMock);
