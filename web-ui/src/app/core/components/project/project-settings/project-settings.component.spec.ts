...
describe('ProjectSettingsComponent', () => {
  ...
  beforeEach(async () => {
    // stub services
    projectsApiServiceStub = jasmine.createSpyObj('projectsApiService', ['deleteProjectMember', 'getProject', 'getProjectMembers']);
  ...
  describe('component rendering', () => {
    ...
    it('should contain components subtext', async () => {
      ...
      // assert
      expect(subText.textContent).toContain('Manage project members.');
    });
    ...
    describe('project members table', () => {
      ...
      it('should call deleteApiKey on clicking delete button in row', async () => {
        // arrange + act also in beforeEach
        projectsApiServiceStub.deleteProjectMember.withArgs(fakeProject.key, fakeProjectMembers[fakeProjectMembers.length - 1].email).and.returnValue(of());
        const deleteButtons: MatButtonHarness[] = await loader.getAllHarnesses(MatButtonHarness.with({ text: 'delete' }));
        ...
        // assert
        fixture.whenStable().then(() => {
          expect(projectsApiServiceStub.deleteProjectMember).toHaveBeenCalledWith(fakeProject.key, fakeProjectMembers[fakeProjectMembers.length - 1].email);
        });
      });
