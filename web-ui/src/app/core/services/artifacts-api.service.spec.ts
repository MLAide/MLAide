...
import 'jasmine';
import { getRandomArtifact, getRandomArtifacts, getRandomCreateOrUpdateModel, getRandomProject } from 'src/app/mocks/fake-generator';
...
describe('ArtifactsApiService', () => {
  ...
  describe('getArtifacts', () => {
    it('should return data source that emits results from api response if isModel is false', async (done) => {
      ...
      const req: TestRequest = httpMock.expectOne(`${service.API_URL}/api/v1/projects/${fakeProject.key}/artifacts?isModel=false`);
  ...
  describe('getArtifactsByRunKeys', () => {
    it('should return data source that emits results from api response', async (done) => {
      ...
      const req: TestRequest = httpMock.expectOne(`${service.API_URL}/api/v1/projects/${fakeProject.key}/artifacts?isModel=false&runKeys=1&runKeys=2&runKeys=3`);
      expect(req.request.method).toBe('GET');
      expect(req.request.params.get('isModel')).toBe('false');
  ...
  describe('putModel', () => {
    it('should execute put request with the model', async (done) => {
      ...
      // assert
      const req: TestRequest = httpMock.expectOne(`${service.API_URL}/api/v1/projects/${fakeProject.key}/artifacts/${fakeArtifact.name}/${fakeArtifact.version}/model`);
  ...
  describe('download', () => {
    it('should return an http response of type arraybuffer as observable from api response if fileId is set', async (done) => {
      ...
      // act
      const run$: Observable<HttpResponse<ArrayBuffer>> = service.download(fakeProject.key, fakeArtifact.name, fakeArtifact.version, fakeArtifact.files[0].fileId);
      ...
      const req: TestRequest = httpMock.expectOne(`${service.API_URL}/api/v1/projects/${fakeProject.key}/artifacts/${fakeArtifact.name}/${fakeArtifact.version}/files/${fakeArtifact.files[0].fileId}`);
      expect(req.request.method).toBe('GET');
    ...
    it('should return an http response of type arraybuffer as observable from api response if fileId is not set', async (done) => {
      ...
      // act
      const run$: Observable<HttpResponse<ArrayBuffer>> = service.download(fakeProject.key, fakeArtifact.name, fakeArtifact.version);
      run$.subscribe(response => {
        expect(response.body).toBe(returnBuffer);
        done();
      });
      ...
      // arrange
      const req: TestRequest = httpMock.expectOne(`${service.API_URL}/api/v1/projects/${fakeProject.key}/artifacts/${fakeArtifact.name}/${fakeArtifact.version}/files`);
      expect(req.request.method).toBe('GET');
      req.flush(returnBuffer);
    });
  });
