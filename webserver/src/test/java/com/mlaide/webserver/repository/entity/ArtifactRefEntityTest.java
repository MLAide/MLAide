...
@Testcontainers
public class ArtifactRefEntityTest {
    @Autowired
    ...
    private ArtifactRefEntity artifactRefEntity;
    ...
    class validation {
        ...
        void invalid_name_should_throw_ConstraintViolationException(String arg) {
            // Arrange
            artifactRefEntity.setName(arg);

            // Act + Assert
            assertThatThrownBy(() -> mongo.save(artifactRefEntity)).isInstanceOf(ConstraintViolationException.class);
        }

        @Test
        void version_is_null_should_throw_ConstraintViolationException() {
            // Arrange
            artifactRefEntity.setVersion(null);
